                    ...
                    if (data.givenDailyReward) {
                        ...
                        if (displayMoneyWon) {
                            ...
                            displayMoneyWon.style.animation =
                                'rewardBounce 5s ease-out forwards'
    ...
    fetch('/getInterestedPeople', {
        ...
        .then((data) => {
            if (data.success && data.interestedUsers) {
                ...
                AllNotifications = AllNotifications.filter(
                    (request) => request.type !== 'interested'
                )
                const interestedWithType = data.interestedUsers.map(
                    (notif) => ({ ...notif, type: 'interested' })
                )
    ...
    fetch('/getAcceptedFriendRequestWebsite', {
        ...
        .then((data) => {
            if (data.success && data.acceptedFriendRequest) {
                ...
                AllNotifications = AllNotifications.filter(
                    (notif) => notif.type !== 'accepted'
                )
                const acceptedWithType = data.acceptedFriendRequest.map(
                    (notif) => ({ ...notif, type: 'accepted' })
                )
...
function arraysEqual(arr1, arr2) {
    ...
    for (let i = 0; i < arr1.length; i++) {
        if (
            arr1[i].fr_id !== arr2[i].fr_id ||
            arr1[i].user_username !== arr2[i].user_username
        ) {
            return false
        }
    }
        ...
        .then((data) => {
            if (data.success && data.unreadCount) {
                ...
                localStorage.setItem(
                    'servedSenderIds',
                    JSON.stringify(updatedServedSenderIds)
                )
                fillUnread(data.unreadCount)
                updateUnreadMessagesForFriends(data.unreadCount)
                cleanupServedSenders(
                    data.unreadCount.map((m) => m.chat_senderId)
                )
            } else {
                numberOfFailsUnred = 0
                globalUnreadCounts = {}
...
function cleanupServedSenders(currentUnreadSenderIds) {
    const servedSenderIds =
        JSON.parse(localStorage.getItem('servedSenderIds')) || []
    const validServedSenderIds = servedSenderIds.filter((id) =>
        currentUnreadSenderIds.includes(id)
    )
    localStorage.setItem(
        'servedSenderIds',
        JSON.stringify(validServedSenderIds)
    )
}
...
function fillUnread(unreadCounts) {
    ...
    if (count > 0) {
        ...
        document.title =
            count === 1 ? `(1) ${originalTitle}` : `(${count}) ${originalTitle}`
    } else {
...
function updateUnreadMessagesForFriends(unreadCounts) {
    ...
    unreadCounts.forEach((unreadCount) => {
        newGlobalUnreadCounts[unreadCount.chat_senderId] =
    ...
    friendElements.forEach((friendElement) => {
        ...
        const friendContainer = document.getElementById(
            `unread_messages_for_friend_container_${friendId}`
        )
        if (!friendContainer) {
        ...
        .then((registration) => {
            ...
            return registration.pushManager.subscribe({
                ...
                applicationServerKey:
                    'BMFWjnjjjer6XUJjuY9RxF_TvRP0Ke6kNNH13oo9lUCRnEYP6ddbpdQ91RD50JexpI8E7THMftePX89bzssMj5Q', // Replace with your public VAPID key
            })
        })
        .then((subscription) => {
    ...
    navigator.serviceWorker.ready.then((registration) => {
        ...
        registration.pushManager.getSubscription().then((subscription) => {
            if (subscription) {
                ...
                console.log(
                    'No previous subscription found, registering fresh...'
                )
                registerServiceWorker(userId)
            }
        })
...
function fillPendingFriendRequest(pendingCount) {
    const container = document.getElementById(
        'pending_friend_requests_container'
    )
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker
            .register('https://ur-sg.com/service-worker.js', {
    ...
    document
        ...
        .addEventListener('click', function (event) {
            ...
            const localPermission = localStorage.getItem(
...
window.addEventListener('load', async function () {
    ...
    const lastRefresh = parseInt(
        localStorage.getItem('notification_last_refresh'),
        10
    )
    const lastBrowserPrefix = localStorage.getItem(
        'notification_browser_prefix'
    )
    const now = Date.now()
    ...
    if (currentEndpoint.startsWith('https://fcm.googleapis.com')) {
        ...
        currentEndpoint.startsWith('https://updates.push.services.mozilla.com')
    ) {
