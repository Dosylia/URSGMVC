...
function updateOnlineStatus(friendList) {
    friendList.forEach((friend) => {
        ...
        if (friendElement && chatName) {
            ...
            const lookingForGame = friendElement.querySelector(
                '.looking-game-status'
            )
    ...
    try {
        ...
        if (friendList.length > 0) {
            ...
            const lookingForButton =
                document.getElementById('looking-for-button')

            if (firstFriend.friend_isLookingGameUser === 1) {
                lookingForButton.style.background =
                    'linear-gradient(45deg, #4CAF50, #66bb6a)'
            } else {
                lookingForButton.style.background =
                    'linear-gradient(135deg, #722084, #b026cf)'
            }
        }

        friendList.forEach((friend) => {
            const friendElement = document.createElement('a')
            friendElement.className = 'username_chat_friend'
            friendElement.href = '#'
            ...
            if (
                ...
                avatarDiv.appendChild(lookingForGame)
            } else if (friend.friend_online === 1) {
                const onlineStatus = document.createElement('span')
                onlineStatus.className = 'online-status'
                avatarDiv.appendChild(onlineStatus)
            }

            const gameLogo = document.createElement('img')
            gameLogo.src =
                friend.friend_game === 'League of Legends'
                    ? 'public/images/league-icon.png'
                    : 'public/images/valorant-icon.png'
